...
trait HasMin
{
     ...
     *
     * The `min` attribute defines the most negative value in the range of permitted values.
     * - If the `value` entered is less than this, the element fails constraint validation.
     * - For date and time inputs, the value must be a valid date/time string.
     * - For numeric inputs, the value must be a number.
     * - This value must be less than or equal to the value of the `max` attribute.
     * - If the `min` attribute is present but is not specified or is invalid, no `min` value is applied.
     * - If the `min` attribute is valid and a non-empty value is less than the minimum allowed, constraint validation
     *   will prevent form submission.
     *
     * There is a special case.
     * - If the data type is periodic (such as for dates or times), the value of `max` may be lower than the value of
     *   `min`, which indicates that the range may wrap around; for example, this allows you to specify a time range
     *   from `10 PM` to `4 AM`.
     *
