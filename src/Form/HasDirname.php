 ...
 *
 * Key features.
 * - Designed for use in form control elements (`<input type="email">`, `<input type="hidden">`,
 *   `<input type="search">`, `<input type="tel">`, `<input type="text">`, and `<input type="url">`).
...
trait HasDirname
{
     ...
     *
     * The `dirname` attribute enables submission of the element's directionality.
     * - When included, the form control will submit with two name/value pairs: the first being the `name` and `value`,
     *   and the second being the value of the `dirname` attribute as the name, with a value of `ltr` (left-to-right) or
     *   `rtl` (right-to-left) as set by the browser.
