 ...
 *
 * Key features.
 * - Designed for use in form control elements (`<input type="email">`, `<input type="password">`, `<input type="tel">`,
 *   `<input type="text">`, `<input type="url">` and `<select>`).
...
trait HasSize
{
     ...
     *   characters (in `em` units) with a default value of `20`.
     * - For `<select>` elements with `multiple` attribute, the `size` attribute determines the number of visible rows
     *   in the list.
     * - For other input types and select, the value represents pixels (in `px` units).
     *
