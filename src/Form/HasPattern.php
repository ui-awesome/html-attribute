 ...
 *
 * Key features.
 * - Designed for use in form control elements (`<input type="email">`, `<input type="password">`,
 *   `<input type="search">`, `<input type="tel">`, `<input type="text">`, `<input type="url">`, and `<textarea>`).
...
trait HasPattern
{
     ...
     * - The pattern must be a valid JavaScript regular expression, as used by the `RegExp` type.
     * - No forward slashes should be specified around the pattern text.
     *
     * When compiling.
     * - The pattern is implicitly wrapped with `^(?:` and `)$`, matching the entire input value.
     * - The `'v'` flag is specified for Unicode code point treatment.
     * - If the `pattern` attribute is present but invalid, no regular expression is applied.
     * - If the pattern is valid and a non-empty value does not match, constraint validation will prevent form
     *   submission.
     *
